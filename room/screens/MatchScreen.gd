...
onready var join_match_id_control := $join_room/entercode/LineEdit
...
func _ready() -> void:
	...
	$Panel/create_button.connect("pressed", self, "_on_match_button_pressed", [OnlineMatch.MatchMode.CREATE])
	$join_room/join_button.connect("pressed", self, "_on_match_button_pressed", [OnlineMatch.MatchMode.JOIN])
...
func _on_match_button_pressed(mode) -> void:
	...
	if Online.nakama_session == null or Online.nakama_session.is_expired():
		get_tree().change_scene("res://main/screens/login.tscn")
